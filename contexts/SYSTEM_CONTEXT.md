...
<!-- provenance: observado -->
- Cobertura de pruebas unitarias limitada al núcleo lógico (`src/http`, `src/services`, `src/stores`, `src/utils`) — 13 archivos de test, 88 casos. Componentes, vistas, router y `src/mocks` no tienen pruebas todavía; extenderla es la continuación natural de este trabajo.
